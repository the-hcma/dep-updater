...
| Remote timeouts and retries cursor rule | yes* | — | `.cursor/rules/remote-timeouts-retries.mdc`: explicit timeouts on every remote call; bounded retries for transient failures only (`Retry-After` / cap / budget); anti-patterns for no-timeout calls and `while True` retries; route ad-hoc `gh` through `scripts/gh-api` (SUGGEST-only during the repository-helpers#608 roll-out) (*missing or invalid FAILS `--new-repo` / `--strict-onboarding`; routine `--all` / `--suggest` emits SUGGEST and succeeds) |
